...
def get_collection_product_permutations():
    ...
    for collection_id, collection_products in collection_to_products.items():
        ...
        product_combinations = list(permutations(unique_products, 3))
        for combo in product_combinations:
            ...
            if is_valid_combo(combo_types):
                ...
                permutation_categories = [
                    item1_type, item2_type, item3_type
                    # , item4_type
                ]
                permutation_categories_all.append(set(permutation_categories))
                categories_counter += permutation_categories
                counter += 1
    random.shuffle(all_collection_permutations)
    permutations_counter = Counter([tuple(per)
                                    for per in permutation_categories_all])
    return all_collection_permutations, all_products, Counter(categories_counter)
